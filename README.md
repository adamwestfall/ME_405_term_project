...
We were tasked with implementing a targeting, aiming, and firing system that would allow our turret to eliminate an enemy before the enemy eliminated us. 
...
Our design focuses on reliability, speed, and precision.
We utilized the entire allotted physical space to create a stable platform rated for the large inertia's being shifted.
...
To adjust the default gun for the duel, auxilary power and data cords were wired through a drilled hole in the base of the handle and soldered to the necessary connections. 
...
A 24V DC motor was also used to provide the necessary speed and torque for pitch control.
...
An emergency stop button breaks the circuit that connects the aforementioned yaw, pitch, and gun motors to the power supply.
...
Indicator LEDs on the control panel allow a user to diagnose the power state of the nerf gun and motors.
...
The A-frame style structure made of plywood supports the gun pitch motors without shaking due to its rigidity and geometry.


![Alt text, alt right, alt left](md_LED_png.png)

Figure 5. Shows the underside electronics which links the STM32 with the pitch and yaw motors, the emergency stop, the reset button, the indicator LEDs, and all gun actuation.



<h1>Software Design:</h1>

The software design philosophy focuses on simplicity and effectiveness.
...
It takes a snapshot and then spits out a pixel array based on temperature.
Our algorithm  first filters the snapshot to set all low temperature  values to zero.
...
The Yaw and Pitch control relies on an encoder tick value to set the desired angles. 
...
Due to unforeseen serious health concerns, our project never had a full opportunity to combine all hardware and software.
...
There was not an opportunity  to combine these separate aspects and run the system as a fully automated turret.
...
If those supports are imbalanced, wobbly, or too heavy, the motor control for both pitch and yaw can become nearly impossible.
Maintaining tension on any belts, gears, or belts is critical to getting reliable movement from the motors.

**Electrical Design:**
Avoid using redundant  components that can potentially fail, such as switches, LEDs, and buttons.
...
Focus energy and effort on the thermal camera targeting system.
The motors, encoders, and controller are straightforward and intuitive.
